...
| T-003 | Phase 1 (MVP): プロジェクト初期化・自動レイアウトエンジン・3画面・ローカル保存 | 高 | レビュー中 | developer | Reviewer指摘1〜5（importDeckJsonのSlide/Block/Asset検証、ErrorBoundary、id衝突時の新規採番、alt属性、画像追加時のエラーハンドリング）を修正済み。詳細はprogress.md参照 |
