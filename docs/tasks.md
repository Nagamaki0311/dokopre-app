...
| T-010 | ダークモード実装 | 高 | レビュー中 | developer | CSS変数2セット化+data-theme切替。スライド面は固定トークンでPNG/PDF出力と一致を維持（D-003）。Playwrightでライト/ダークのPNG出力ハッシュ一致・テーマ即時反映・リロード後の永続化を確認、npm test/build/gradle assembleDebug成功 |
