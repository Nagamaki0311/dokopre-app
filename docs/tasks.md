...
| T-013 | 自動レイアウトへの整列軸（左右中央揃え含む）追加 | 中 | 実装中 | developer | Reviewer実証検証でCritical/High/Medium指摘なし。Low1件（LayoutResult.alignが実質未消費、boxesのx/w補正で視覚効果は完結）+Nit1件の軽微な仕上げ対応中 |
